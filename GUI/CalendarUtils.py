...
class CalendarTickter:
    ...
    def __place_widgets(self):
        ...
        gframe.pack(in_=self.G_Frame, fill=tkinter.X, pady=5)
        ...
        tkinter.Label(hframe, text = '年', justify = 'left').grid(in_=hframe, column=2, row=0, padx=(0,5))
        ...
        tkinter.Label(hframe, text = '月', justify = 'left').grid(in_=hframe, column=4, row=0)
...
if __name__ == '__main__':
    ...
    date.place(x = 0, y = 0, relx = 5/20, rely = 1/6, width = 10, heigh = 20)

    #CalendarTickter((x, y), 'ur').selection() 获取日期，x,y为点坐标
    date_str_gain = lambda: [
        date_str.set(date)
        for date in [CalendarTickter((x, y), 'ur').selection()] 
        if date]
